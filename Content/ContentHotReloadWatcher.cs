...
{
    /// <summary>
    /// 通用的目录轮询热重载工具：按固定间隔比较文件修改时间，变化（含首次出现）时回调一次文件路径，取代各模块各自实现的轮询热重载。
    /// 刻意不用 <see cref="FileSystemWatcher"/>——Unity/BepInEx 环境下文件锁与事件丢失更常见；只做"文件变了"这一件事，解析、编译、写回目录都留给调用方的回调。
    /// </summary>
